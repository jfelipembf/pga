...
import { Row, Col, Button, Label, Input, FormFeedback, FormGroup, Spinner } from "reactstrap";
...
const CompanyForm = ({
    ...
    const { handleCepBlur, isLoadingCep } = useAddressLookup(formik);

    // Busca automática ao digitar os 8 dígitos do CEP
    React.useEffect(() => {
        const cep = formik.values.zipCode?.replace(/\D/g, '');
        if (cep?.length === 8) {
            handleCepBlur({ target: { value: formik.values.zipCode } });
        }
    }, [formik.values.zipCode, handleCepBlur]);
                                ...
                                <div key={index} className="row mb-3 align-items-center">
                                    ...
                                    <Col md="2" className={index === 0 ? "mt-4" : ""}>
                                        <Button
                                            ...
                                            disabled={formik.values.managers.length <= 1}
                                        >
